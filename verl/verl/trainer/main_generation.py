...
def main(config):
    ...
    if os.path.exists(config.data.output_path):
        raise Exception(f"Output file {config.data.output_path} already exists. Skipping generation and proceeding to evaluation.")
        try:
    ...
    for i in range(total):
        ...
        for r in response_lst:
            token_len = len(tokenizer.encode(r))
            if token_len != config.rollout.response_length:
    ...
    row_data = {
        ...
        'n': n_samples,
        'pass@1': f"{pass_at_1:.3f}",
        f'pass@n': f"{pass_at_n:.3f}",
        'tokens': token_mean,
        'max token': token_max
    }
