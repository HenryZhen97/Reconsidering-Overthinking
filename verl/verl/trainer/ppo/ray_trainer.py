...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        reward_tensor_lst = []
        data_source_lst = []
        external_redundancy_lst = []
        internal_redundancy_lst = []
        for test_data in self.val_dataloader:
            ...
            repeat_map = {
                "aime24": self.config.actor_rollout_ref.rollout.n_val_aime,
                "math500": self.config.actor_rollout_ref.rollout.n_val_math,
                "gsm8k": self.config.actor_rollout_ref.rollout.n_val_gsm8k,
            }
            test_batch = test_batch.repeat_by_source(repeat_map)
            ...
            reward_tensor, metrics = self.val_reward_fn(test_batch)

            reward_tensor_lst.append(reward_tensor)
            external_redundancy_lst.append(metrics["external_redundancy"])
            internal_redundancy_lst.append(metrics["internal_redundancy"])
            data_source_lst.append(test_batch.non_tensor_batch.get('data_source', ['unknown'] * reward_tensor.shape[0]))

        reward_tensor = torch.cat(reward_tensor_lst, dim=0).sum(-1).cpu()  # (batch_size,)
        data_sources = np.concatenate(data_source_lst, axis=0)
        external_redundancy = np.concatenate(external_redundancy_lst, axis=0)
        internal_redundancy = np.concatenate(internal_redundancy_lst, axis=0)

        # evaluate test_score based on data source
        data_source_reward = {}
        external_redundancy_dict = {}
        internal_redundancy_dict = {}
        for i in range(reward_tensor.shape[0]):
            ...
            internal_redundancy_dict[data_source].append(internal_redundancy[i])

        metric_dict = {}
        for data_source, rewards in data_source_reward.items():
            metric_dict[f'val/test_score/{data_source}'] = np.mean(rewards)

        for data_source, redundancy in external_redundancy_dict.items():
            redundancy = [r for r in redundancy if r]
            metric_dict[f'val/external_redundancy/{data_source}'] = np.mean(redundancy) if redundancy else 0.0
        
        for data_source, gain in internal_redundancy_dict.items():
            gain = [g for g in gain if g]
            metric_dict[f'val/internal_redundancy/{data_source}'] = np.mean(gain) if gain else 0.0
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        # Log to metrics
                        metrics['batch/solve_none'] = round(solve_none/len(unique_uids), 2)
                        metrics['batch/solve_all'] = round(solve_all/len(unique_uids), 2)
                        metrics['batch/solve_partial'] = round((len(unique_uids) - solve_none - solve_all) / len(unique_uids), 2)
