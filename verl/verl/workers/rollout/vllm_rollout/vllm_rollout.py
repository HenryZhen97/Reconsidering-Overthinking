...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto,  **kwargs) -> DataProto:
        ...
        if self.reward_fn is not None and not is_validation:
            reward_tensor, metrics = self.reward_fn(batch)
            # remove 0 scores
            redundancy = [r for r in metrics['external_redundancy'] if r]
            gain = [g for g in metrics['internal_redundancy'] if g]
            # calculate mean scores
            batch.batch['token_level_scores'] = reward_tensor
            batch.meta_info['external_redundancy'] = np.array(redundancy).mean() if redundancy else 0.0
            batch.meta_info['internal_redundancy'] = np.array(gain).mean() if gain else 0.0
