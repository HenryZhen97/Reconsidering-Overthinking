...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_rollout(self):
        ...
        if self.reward_config is not None and self.config.rollout.compute_reward:
            ...
            from rllm.rewards.reward_manager import VeRLRewardManager
            from rllm.rewards.compress_reward import compress_compute_score, compress_val

            if self.config.actor.compress:
                print("-" * 30)
                print("Start compress reward.")
                print("-" * 30)
                self.reward_fn = VeRLRewardManager(tokenizer=self.tokenizer, reward_func=compress_compute_score, train_config=self.config)
                self.val_reward_fn = VeRLRewardManager(tokenizer=self.tokenizer, reward_func=compress_val, train_config=self.config)
            else:
