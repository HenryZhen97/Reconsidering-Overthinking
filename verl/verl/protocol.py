...
class DataProto:
    ...
    def repeat_by_source(self, repeat_mapping: dict[str, int]) -> "DataProto":
        """
        Repeat each sample in the batch by a number of times based on its 'data_source' field.

        Args:
            repeat_mapping (dict): Mapping from data_source value to repeat count.

        Returns:
            DataProto: A new DataProto with per-sample repeated data.
        """
        data_sources = self.non_tensor_batch['data_source']  # shape: (N,)
        assert isinstance(data_sources, np.ndarray), "data_source must be a numpy array"
        assert data_sources.shape[0] == next(iter(self.batch.values())).shape[0], "Length mismatch"

        # Step 1: 构造每个样本对应的 repeat 次数数组
        repeat_counts = np.array([repeat_mapping[source] for source in data_sources])

        # Step 2: repeat_interleave tensor batch
        repeated_tensors = {
            key: tensor.repeat_interleave(torch.tensor(repeat_counts).to(tensor.device), dim=0)
            for key, tensor in self.batch.items()
        }

        repeated_batch = TensorDict(
            source=repeated_tensors,
            batch_size=(repeat_counts.sum(),),
        )

        # Step 3: repeat non_tensor_batch
        repeated_non_tensor_batch = {
            key: np.repeat(val, repeat_counts, axis=0)
            for key, val in self.non_tensor_batch.items()
        }
